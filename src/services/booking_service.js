...
async function getCustomerBookingsService(id) { return await getCustomerBookings(id); }
async function getProviderBookingsService(id) { return await getProviderBookings(id); }

async function cancelBookingService(bookingId, userId) {
    const booking = await findBookingById(bookingId);
    if (!booking || booking.customer_id.toString() !== userId) throw { reason: "Unauthorized", statusCode: 403 };
    if (booking.status === 'Cancelled') throw { reason: "Already cancelled", statusCode: 400 };
...
async function updateBookingStatusService(bookingId, providerId, status) {
    const booking = await findBookingById(bookingId);
    if (!booking || booking.provider_id.toString() !== providerId) throw { reason: "Unauthorized", statusCode: 403 };
    const valid = { 'Pending': ['Confirmed'], 'Confirmed': ['In Progress', 'Completed'], 'In Progress': ['Completed'] };
    if (!valid[booking.status]?.includes(status)) throw { reason: "Invalid status", statusCode: 400 };
...
module.exports = { getCustomerBookingsService, getProviderBookingsService, cancelBookingService, updateBookingStatusService ,createBookingService,      // ← NEW
